// You have been employed by the Japanese government to write a function that tests whether
// or not a building is strong enough to withstand a simulated earthquake.
...
// A building begins with a strength value of 1000 when first built,
// but this value is subject to exponential decay of 1% per year. For more info on exponential decay,
//  follow this link - https://en.wikipedia.org/wiki/Exponential_decay
...
function strongEnough(earthquake, age) {
  const earthquakePower = earthquake.reduce((acc, val) => {
    return acc * val.reduce((acc, val) => acc + val, 0);
  }, 1);

  const buildingStrength = 1000 * Math.exp(-0.01 * age);

  return buildingStrength > earthquakePower ? 'Safe!' : 'Needs Reinforcement!';
}
